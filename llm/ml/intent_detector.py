...
if __name__ == "__main__":
    ...
    # Train
    print("\n[TRAINING PHASE]")
    metrics = detector.train(str(dataset_path), test_size=0.2, random_state=42)
    ...
    test_cases = [
        ...
        "Teşekkür ederim",
        "SSH nedir",
        "Ubuntu SSH hardening scripti oluştur",
        "Bugün hava nasıl",
        "Yardım lazım",
        "Görüşürüz"
    ]
    ...
    for text in test_cases:
        ...
        sorted_probs = sorted(intent.probabilities.items(), key=lambda x: x[1], reverse=True)[:3]
        for cls, prob in sorted_probs:
            print(f"    {cls}: {prob:.4f}")

    print("\n" + "="*70)
    print("[COMPLETE] Models trained and saved successfully!")
    print("="*70)
