...
if __name__ == "__main__":
    print("="*70)
    print("PARAMETER INFERENCE ENGINE - TEST")
    print("="*70)

    engine = ParameterInferenceEngine(debug=True)

    test_cases = [
        # OS inference
        ("Ubuntu 22.04'te SSH nasıl yapılandırılır?", None),
        ("Windows Server 2022 firewall ayarları", None),
        ("CentOS 9 hardening script yaz", None),

        # Role inference
        ("Developer olarak SSH key yönetimi", None),
        ("DevOps için CI/CD pipeline güvenliği", None),
        ("Security analyst olarak log analizi", None),

        # Security level
        ("Maximum güvenlik için SSH hardening", None),
        ("Basit firewall kuralları yeterli", None),

        # Session context override
        ("Peki firewall?", {'os': 'ubuntu_22_04', 'role': 'developer'}),
    ]

    for question, session_ctx in test_cases:
        print(f"\n{'─'*70}")
        print(f"Question: {question}")
        if session_ctx:
            print(f"Session Context: {session_ctx}")

        result = engine.infer_all(question, session_ctx)

        print(f"\nInferred Parameters:")
        print(f"  OS: {result['os']} (source: {result['inference_source']['os']})")
        print(f"  Role: {result['role']} (source: {result['inference_source']['role']})")
        print(f"  Security Level: {result['security_level']} (source: {result['inference_source']['security_level']})")

        msg = engine.get_inference_message(result)
        if msg:
            print(f"\nUser Message:\n{msg}")
