...
if __name__ == "__main__":
    ...
    # Initialize logger
    logger = get_logger(
        log_file="logs/test_chatbot.log",
        metrics_file="logs/test_metrics.jsonl",
    ...
    for i in range(3):
        ...
        trace = RequestTrace(
            trace_id=trace_id,
            user_question=f"Test question {i+1}",
            intent="os_hardening",
            complexity=["simple", "medium", "complex"][i % 3],
            path=["simple", "medium", "complex"][i % 3],
            provider="groq",
            model="llama-3.1-8b",
            response_length=150 + i * 50,
            latency_ms=timer.elapsed_ms,
            cost_usd=0.0002 * (i + 1),
            quality_score=85.0 + i * 5,
            validation_passed=True
        )

        logger.log_request(trace)
    ...
    logger.log_error(
        ...
        context={"provider": "groq", "retry_count": 3}
    )
    ...
    print("="*70)
    print(f"Logs written to: logs/test_chatbot.log")
    print(f"Metrics written to: logs/test_metrics.jsonl")
